...
export class ConceptMainService {
  ...
  public getMainConceptThings(query:any):Observable<any> {
    return this.http.get(`http://localhost/consumer/api/v1/main/things`, query).map((res:any)=> {
  ...
  public getMainConceptImages(query:any):Observable<any> {
    return this.http.get(`http://localhost/consumer/api/v1/main/images/${query.thingId}`).map((res:any)=> {
      let parseRes = JSON.parse(res._body);
