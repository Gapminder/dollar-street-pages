...
export class DataProvider {
  ...
  public static matrixPageText:any = {
    'H2BYInHeader': {
      element: ():string => element.all(by.css('div[class*="some-filter-container"]>span')).first().getText(),
      actualResult: 'in'
    },
    'H2INInHeader': {
      element: ():string => element.all(by.css('div[class*="some-filter-container"]>span')).last().getText(),
      actualResult: 'by income'
    },
    'PoorestHeaderOnStreetWidget': {
      element: ():string => $('text[class="poorest"]').getText(),
