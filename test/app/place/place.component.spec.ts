...
describe('PlaceComponent', () => {
  ...
  it('must init', () => {
    ...
    expect(context.query).toEqual(`thing=Home&place=54b6866a38ef07015525f5be&image=54b6862f3755cbfb542c28cb`);
  ...
  it('choseCurrentPlace', () => {
    ...
    expect(context.changeLocation.calls.argsFor(0)).toEqual([streetPlacesData[0], 'Home']);
  });
  it('changeLocation', () => {
    context.init = false;
    ...
    context.changeLocation(place, 'Home');
    ...
    expect(context.urlChangeService.replaceState.calls.argsFor(0))
      .toEqual(['/place', `thing=Home&place=${place._id}&image=${place.image}`]);
